...
def compute_acc(planet):
    ...
    for other in planets:
        if planet.name == other.name:           # ensuring the planet isnt using itself to accelerate
            ...
            planet.energy = 2 * kinetic_energy / abs(potential_energy)
    return accx, accy
####################################
def run_step(event):
    planet = event.planet
...
if __name__ == "__main__":
    ...
    #while planets[-1].x_pos > 0 or i == 0:
        ...
        for e in planets:
            energy = energy + e.energy * e.instances
        ...
        if i % 100 == 0 and i!=0 and i<2000:
            ...
            for num, planet in enumerate(iterate_planets):
                ...
                if checked == False and planet.shrink != True and (planet.vel / planet.prev_vel) > 0.9999 and abs(planet.vel / planet.prev_vel) < 1.0001:# and distance(0,0,planet.x_pos,planet.y_pos)>5e11:
                    checked = True
